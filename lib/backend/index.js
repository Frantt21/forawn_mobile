...
const CacheDatabase = require('./database');
...
app.get('/cache/check', async (req, res) => {
  ...
  try {
    const cachedSong = cacheDB.getSong(title, artist);
...
app.get('/cache/stats', (req, res) => {
  try {
    const dbStats = cacheDB.getStats();
...
app.post('/cache/cleanup', async (req, res) => {
  try {
    const oldSongs = cacheDB.cleanupOldSongs(CACHE_EXPIRATION_DAYS);
...
// Run cleanup on startup (after 1 minute to let everything initialize)
